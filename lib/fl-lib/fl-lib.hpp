...
static inline void fl_ring_fifo_reset(fl_ring_fifo_t * obj)
{
    obj->rdIdx = 0;
    obj->wrIdx = 0;
}
...
/**
 */
#define fl_linked_list_find(ptr_to_current, field, comparator_func, ...) \
({ \
        fl_auto_type(ptr_to_current) ___item = ptr_to_current; \
        while (___item) \
        { \
        	if (comparator_func(__VA_ARGS__, ___item) == true) break; \
